...
class ConvCat42:
    def __init__(self, window, filename, result_dir, encoding):
        self.window = window
        self.filename = filename
        self.encoding = encoding
        self.db = ConvHelpers(result_dir, 'cat42.db3')
        window.refresh()
        self.db3_open()
        window.refresh()
        self.cat42Read()
...
CREATE TABLE s001 (
  ord int primary key,
  todo1, todo2, data text, chv_doc text, ecf_fab text,
  cod_mod text, ser text, num_doc int, cod_part text,
  cfop int, num_item int, cod_item text, ind_oper int,
  cod_legal int, num_linha int, e_s, sinal,
  qtd_ent real, icms_tot_ent real,
  qtd_sai real, icms_uni_sai real,
  icms_cod_legal1 real, icms_cod_legal2 real, icms_cod_legal3 real,
  icms_cod_legal4 real, icms_sai_comercial real,
  icms_confr_sai_consumidor real, icms_confr_sai_demais_hip real,
...
CREATE TABLE s010 (
  ord int primary key,
  todo1, todo2, cod_legal int,
  icms_cod_legal1 real, icms_cod_legal2 real, icms_cod_legal3 real, icms_cod_legal4 real,
  icms_sai_comercial real, icms_confr_sai_consumidor real, icms_confr_sai_demais_hip real,
  val_ressarc real, val_complem real, cred_op_proprias real)
""")
...
CREATE TABLE s100 (
  ord int primary key,
  COD_REG, DATA_HORA, TIPO_MSG, NUM_MSG CAMPO, VALOR_CAMPO, DESCRICAO, todo, LINHA, TIPO_REG)
""")
    ...
    def cat42ReadLine(self, linha):
        ...
        # EITA.... 5001 e 5010 estão com real em formato inglês .... 99,999.99
        if campos[1] == '5001':
